 ...
 *
 */
public class MyDynamicListActivity extends BaseActivity implements OnClickListener,OnRefreshListener,
OnLoadListener{
	...
	protected void getData() {
		...
		isRefresh = true;
		if(isNetConnection()){
			GetDataPost.getInstance(this).getMyDynamicList(handler,String.valueOf(currentPage), pageNum);
		}
	}
	...
	public void onSuccess(String sign, Bundle bundle) {
		if (bundle != null) {
			...
			.getSerializable(Protocol.DATA);
			 ...
			 if(mAdapter == null){
				 mAdapter = new MyDynamicAdapter(MyDynamicListActivity.this, allDynamicListBeans);
						...
						public void onItemClick(AdapterView<?> parent, View view,
							...
							if(position<=allDynamicListBeans.size()){
								itemBean = allDynamicListBeans.get(position-1);
								 Log.i("cm_socket", "size"+allDynamicListBeans.size()+"position"+position);
								Intent in = new Intent();
								in.putExtra("releaseId", itemBean);
								//in.putExtra("releaseId", itemBean.getPk_release());//新闻id
								in.setClass(MyDynamicListActivity.this, MyDynamicDetailActivity.class);
								startActivity(in);
							}
